...
function createHydrationFunctions(rendererInternals) {
  const { mt: mountComponent, p: patch, o: { patchProp: patchProp2, nextSibling: nextSibling2, parentNode: parentNode2, remove: remove2, insert: insert2, createComment: createComment2 } } = rendererInternals;
    ...
    switch (type) {
      ...
      default:
        if (shapeFlag & 1) {
          ...
          nextNode = isFragmentStart ? locateClosingAsyncAnchor(node) : nextSibling2(node);
  ...
  const hydrateFragment = (node, vnode, parentComponent, parentSuspense, slotScopeIds, optimized) => {
    ...
    const container = parentNode2(node);
    const next = hydrateChildren(nextSibling2(node), vnode, container, parentComponent, parentSuspense, slotScopeIds, optimized);
    if (next && isComment(next) && next.data === "]") {
      return nextSibling2(vnode.anchor = next);
    } else {
      hasMismatch = true;
      insert2(vnode.anchor = createComment2(`]`), container, next);
      return next;
    }
  };
  ...
  const setScopeId2 = (el, vnode, scopeId, slotScopeIds, parentComponent) => {
    ...
    if (parentComponent) {
      ...
      if (vnode === subTree) {
        ...
        setScopeId2(el, parentVNode, parentVNode.scopeId, parentVNode.slotScopeIds, parentComponent.parent);
      }
    }
  };
...
class HElement {
  ...
  getElementById(id) {
    var _a;
    if (this.id === id || ((_a = this.props) == null ? void 0 : _a.id) === id) {
      return this;
    }
  ...
  markDirty() {
    ...
    nextTick(() => {
      ...
      const app = this.app;
      if (app) {
        const flushElements = app._context.provides[flushElementsKey];
        if (flushElements) {
          try {
...
function insert(child, parent, ref2) {
  let refIndex;
  if (ref2) {
    refIndex = parent.children.indexOf(ref2);
    if (refIndex === -1) {
      console.error("ref: ", ref2);
      console.error("parent: ", parent);
      throw new Error("ref is not a child of parent");
    }
  }
  remove(child, false);
  refIndex = ref2 ? parent.children.indexOf(ref2) : -1;
  if (refIndex === -1) {
...
function attachToPage(el, pageId) {
  el.pageId = pageId;
  el.markDirty();
}
...
let fragmentId = 1;
function encodePageUpdates(dirtyElements2) {
  const changes = [];
  const changedFragmentIds = /* @__PURE__ */ new Set();
  const changedPageIds = /* @__PURE__ */ new Set();
  for (const elem of dirtyElements2) {
...
const { render: baseRender, createApp: baseCreateApp } = createHydrationRenderer(nodeOps);
const createApp = (rootComponent, rootProps) => {
  const app = baseCreateApp(rootComponent, rootProps);
  const { mount } = app;
  app.mount = (rootContainer) => {
    return mount(rootContainer, rootContainer.hasChildNodes());
  };
  return app;
};
const $app = {
...
const routes = [{ "name": "About", "path": "/about", "component": () => import("./About.js"), "props": true }, { "name": "all", "path": "/:all(.*)*", "component": () => import("./_...all_.js"), "props": true }];
$app.create = () => {
  ...
  const app = createApp(defineComponent({
    ...
    render() {
      return /* @__PURE__ */ h(RouterView, null);
    }
  }));
  app.use(router);
  return { app, router };
...
const pages = {};
async function onPageLoad(pageId, url) {
...
function _onPageLoad(app, root, pageId) {
  pages[pageId] = { app, root };
  app.mount(root);
  nodeOps.attachToPage(root, pageId);
}
function onPageUnload(pageId) {
  delete pages[pageId];
}
function triggerEvent(pageId, elementId, event, options) {
  ...
  elem.triggerEvent(__spreadProps(__spreadValues({}, event), {
    bubbles: options == null ? void 0 : options.bubbles,
    stopPropagation() {
      this.bubbles = false;
    }
  }));
}
export { _onPageLoad as _, onPageUnload as a, defineComponent as d, h, onPageLoad as o, triggerEvent as t };
